...
function notifier() {
    ...
    setTimeout(function() {
        location.reload();
    }, 1000 * 60 * 60 * 3); // Refresh after 3 hours
    ...
    audio = function() {
        return Number(localStorage.getItem("sound"));
    };
    ...
    if (localStorage.getItem("alt2") && altEnabled) {
        ...
        extraAltMsgs();
        setInterval(extraAltMsgs, 120000);
    }
    ...
    apireq.onreadystatechange = function() {
        if (apireq.readyState === 4 && apireq.status === 200) {
            ...
            if (altMsg > document.getElementById("altMsgNum").innerText && firsttime === false && altNotifications()) {
                ...
                if (notifClose !== 0) setTimeout(function() {
                    notification.close();
                }, notifClose);
...
function notifyComment(author, content, Id, profilePic) {
    ...
    notification.onclick = function() {
        ...
        notification.close();
        openLink("https://scratch.mit.edu/messages/");
        closeTabOnClear(function() {
            messagesTab.location.replace("https://scratch.mit.edu/users/" + user + "/#comments-" + Id);
        });
    };
}
    ...
    if (document.getElementById("settSFX").value == "url") {
        ...
        if (document.getElementById("inputurl").value != "") {
            ...
            newsfx.onended = function() {
                ...
                document.getElementById("play").setAttribute("onclick", document.getElementById("play").getAttribute("onclick").substring(2));
                document.getElementById("play").style.textDecoration = "underline";
            }
        }
    } else {
